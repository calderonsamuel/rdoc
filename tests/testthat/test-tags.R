...
test_that("@typed-return parses correctly", {
  tag <- list(raw = "{class_numeric[1]} the mean value")
...
test_that("@typed-return handles empty description", {
  tag <- list(raw = "{class_data.frame}")

  result <- tag_parse_typed_return(tag)

  expect_equal(result$val$type, "class_data.frame")
  expect_equal(result$val$description, "")
})

test_that("@typed-return handles union types", {
  tag <- list(raw = "{NULL | class_numeric} result or NULL if invalid")

  result <- tag_parse_typed_return(tag)

  expect_equal(result$val$type, "NULL | class_numeric")
