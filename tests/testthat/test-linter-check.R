...
test_that("linter flags type mismatch - literal argument", {
  ...
  code <- "
    #' @typedParam x {class_numeric} number input
  ...
  expect_true(any(grepl("class_integer.*class_double.*class_character", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter handles named arguments", {
  ...
  code <- "
    ...
    #' @typedParam text {class_character} text
...
test_that("linter handles mixed positional and named arguments", {
  ...
  code <- "
    #' @typedParam x {class_numeric} number
    #' @typedParam y {class_numeric} another number
    #' @typedParam z {class_character} text
...
test_that("linter handles multiple lints in single call", {
  ...
  code <- "
    #' @typedParam x {class_numeric} number
    #' @typedParam y {class_numeric} another number
    #' @typedParam z {class_numeric} third number
...
test_that("linter extracts types from accumulated comments", {
  ...
  code <- "
    #' Function with types
    #' @typedParam x {class_numeric} a number
    calculate <- function(x) x * 2
...
test_that("linter infers type from logical variable", {
  ...
  expect_equal(length(lints), 1)
  expect_true(any(grepl("class_integer.*class_double.*class_logical", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter infers type from NULL variable", {
  ...
  expect_equal(length(lints), 1)
  expect_true(any(grepl("class_integer.*class_double.*NULL", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter infers type from list() constructor", {
  ...
  expect_equal(length(lints), 1)
  expect_true(any(grepl("class_integer.*class_double.*class_list", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter infers type from data.frame() constructor", {
  ...
  expect_equal(length(lints), 1)
  expect_true(any(grepl("class_integer.*class_double.*data.frame", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter infers type from matrix() constructor", {
  ...
  expect_equal(length(lints), 1)
  expect_true(any(grepl("class_integer.*class_double.*matrix", vapply(lints, function(l) l$message, character(1)))))
})
...
test_that("linter passes when list type matches", {
  ...
  code <- "
    #' @typedParam x {class_list} a list
    foo <- function(x) x
...
test_that("linter passes when data.frame type matches", {
  ...
  code <- "
    #' @typedParam x {class_data.frame} a data frame
    foo <- function(x) x
...
test_that("linter infers types from chained function calls", {
  ...
  code <- "
    #' @typedReturn {class_data.frame} a data frame
    load_data <- function() data.frame(x = 1:3)

    #' @typedParam df {class_data.frame} input data
    #' @typedReturn {class_list} processed results
    process_data <- function(df) list(df)

    #' @typedParam x {class_list} input list
...
test_that("linter infers types from functions returning list, data.frame, matrix", {
  ...
  code <- "
    ...
    #' @typedParam x {class_list} list input
    use_list <- function(x) x

    #' @typedParam x {class_data.frame} df input
    use_df <- function(x) x
